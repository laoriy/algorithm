/**
 * 题目描述假如有一排房子，共 n 个，每个房子可以被粉刷成红色、蓝色或者绿色这三种颜色中的一种，你需要粉刷所有的房子并且使其相邻的两个房子颜色不能相同。
...
function minCost(costs) {
    ...
    for (let i = 1; i < costs.length; i++) {
        dp[i][0] = Math.min(dp[i - 1][1], dp[i - 1][2]) + costs[i][0] // 第1个房子涂第1种颜色
        dp[i][1] = Math.min(dp[i - 1][0], dp[i - 1][2]) + costs[i][1] // 第1个房子涂第2种颜色
        dp[i][2] = Math.min(dp[i - 1][0], dp[i - 1][1]) + costs[i][2] // 第1个房子涂第3种颜色
    }
