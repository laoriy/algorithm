...
var getAncestors = function (n, edges) {
    ...
    for ([parent, child] of edges) {
        group[parent].push(child) // group[parent] 代表 parent 的直接子节点
    }
     const vis = Array(n).fill(-1);
    function dfs(i, ancestor) {
        vis[i] = ancestor  // 避免重复访问
        for (let x of group[i]) {
            if (vis[x] != ancestor) {
                ans[x].push(ancestor) // 每个子节点都加一下祖先
                dfs(x, ancestor) // 下面的所有子节点都要加祖先
            }
        }
    }
